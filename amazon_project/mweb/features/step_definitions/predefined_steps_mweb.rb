...
#   end
#   # if @params['device_type'].include? "Android" and element.include? 'button'
...
And /^I enter "([^"]*)" in text field with "?(.*?)"? "([^"]*)"$/ do |value, how, what|
  ...
  puts how
  puts value
  @browser.text_field(how.to_sym, /#{what}/).when_present.set(value)
end
...
#   #what = Regexp.new(Regexp.escape(what)) unless how =~ /index|text/i or what.is_a?(Regexp)
#   @browser.select(how.to_sym, what).select(/#{value}/)
# end
